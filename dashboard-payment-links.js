...
class PaymentLinksAdmin {
  ...
  bind() {
    ...
    app.querySelectorAll('[data-copy]').forEach((button) => button.onclick = async () => { await navigator.clipboard.writeText(button.dataset.copy); this.toast('Message et lien copiés.'); });
  ...
  async productPicker(selectProduct) { const modal = this.modal('<div class="panel-head"><h2>Rechercher une image produit</h2><button class="button secondary small" data-close>Fermer</button></div><input class="product-search" placeholder="Saisissez un nom, SKU ou catégorie"><div class="product-results"><div class="empty">Commencez votre recherche pour afficher un produit.</div></div>'); const search = modal.querySelector('.product-search'); const results = modal.querySelector('.product-results'); const load = async () => { if (search.value.trim().length < 2) { results.innerHTML = '<div class="empty">Saisissez au moins 2 caractères.</div>'; return; } results.textContent = 'Recherche…'; try { const products = (await this.api('managePaymentLinks', { method: 'POST', body: JSON.stringify({ action: 'products', search: search.value }) })).products || []; results.innerHTML = products.map(p => `<button type="button" class="product-choice" data-product='${esc(JSON.stringify(p))}'><span><strong>${esc(p.name)}</strong><small>${esc([p.sku,p.category].filter(Boolean).join(' · ') || 'Produit')}</small></span></button>`).join('') || '<div class="empty">Aucun produit trouvé.</div>'; results.querySelectorAll('[data-product]').forEach(button => button.onclick = () => { selectProduct(JSON.parse(button.dataset.product)); modal.remove(); }); } catch (error) { results.textContent = error.message; } }; modal.querySelector('[data-close]').onclick = () => modal.remove(); let timer; search.oninput = () => { clearTimeout(timer); timer = setTimeout(load, 250); }; }
