...
class PaymentLinksAdmin {
  ...
  render() {
    ...
    const rows = visibleLinks.map((link) => {
      const status = this.status(link); const url = `https://us-central1-smartcutservices-9ce54.cloudfunctions.net/paymentLinkSharePage?ref=${encodeURIComponent(link.reference)}`;
