...
function getLineDeliveryFee(item = {}, order = {}) {
  ...
  const matched = details.find((entry) => {
    ...
    if (vendorId && entry.vendorId && entry.vendorId === vendorId && itemName && normalizeText(entry.productName) === itemName) return true;
    return false;
  });
  ...
  return (Number(matched.unitFee) || 0) * quantity;
}
...
export function buildVendorSalesSummary({
  ...
  orders.forEach((order) => {
    ...
    // Older orders can contain only one vendor-level delivery amount. Allocate
    // the remaining fee across that vendor's lines without touching commission.
